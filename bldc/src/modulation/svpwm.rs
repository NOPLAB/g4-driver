...
use crate::transforms::inverse_clarke;
...
pub fn calculate_svpwm(v_alpha: f32, v_beta: f32, v_dc: f32, max_duty: u16) -> (u16, u16, u16) {
    ...
    // Formula: duty = (value + 1.0) / 2.0 * max_duty + 0.5 (rounding)
...
pub fn calculate_sinusoidal_pwm(
    ...
    // Convert from alpha-beta to three-phase
...
mod tests {
    ...
    #[test]
    fn test_svpwm_zero_dc_voltage() {
        let (du, dv, dw) = calculate_svpwm(6.0, 3.0, 0.0, 100);
        // Should return 50% duty cycle
        assert_eq!(du, 50);
        assert_eq!(dv, 50);
        assert_eq!(dw, 50);
    }
    ...
    #[test]
    fn test_svpwm_duty_range() {
        // Test that duty cycles stay within valid range
        for alpha in [-10.0, -5.0, 0.0, 5.0, 10.0] {
            for beta in [-10.0, -5.0, 0.0, 5.0, 10.0] {
                let (du, dv, dw) = calculate_svpwm(alpha, beta, 12.0, 100);
                assert!(du <= 100);
                assert!(dv <= 100);
                assert!(dw <= 100);
            }
    ...
    #[test]
    fn test_sinusoidal_pwm_zero_dc_voltage() {
        let (du, dv, dw) = calculate_sinusoidal_pwm(6.0, 3.0, 0.0, 100);
        // Should return 0 duty cycle
        assert_eq!(du, 0);
        assert_eq!(dv, 0);
        assert_eq!(dw, 0);
    }
